...
This document is the authoritative implementation contract for `crates/nooboard-core`.
...
- exposing a frontend-safe Rust API for the GUI application
...
The legacy `crates/nooboard-app` and `crates/nooboard-sync` crates have been removed from the
active workspace.
...
- `nooboard-core` MUST expose a standalone Rust API that the GUI crate can use directly.
...
The clipboard-history paging contract in `nooboard-core` MUST mirror the directional storage
...
### 6.1 Clipboard history query scaling

The current clipboard history API is sufficient for the existing GUI, but it is not yet the final
scalable history contract.

Current behavior:

- `list_clipboard_history` returns full `ClipboardRecord` rows
- pagination uses one directional `direction + anchor + limit` contract
- the page return type includes `has_more` and `next_anchor`
- the anchor advances only when a page is successfully loaded

Planned upgrade path for scalable GUI history:

- `nooboard-core` SHOULD continue to use `ClipboardRecord` as the clipboard-history row/detail
  model unless a future spec revision explicitly introduces a second history DTO
- if clipboard history virtualization and bounded page eviction are implemented in the GUI,
  `nooboard-core` MUST expose stable anchors that remain valid when newer head items are inserted
- `Older + anchor=None` MUST load the history head page
- `Older + anchor=Some(anchor)` MUST load records strictly older than `anchor`
- `Newer + anchor=Some(anchor)` MUST load records strictly newer than `anchor`
- `Newer + anchor=None` MUST return an explicit error
- if bounded page eviction is introduced in the GUI, `nooboard-core` MUST make it possible to
  recover evicted history pages without forcing the GUI to reload the entire list from the head

Anchor semantics that MUST remain true:

- a history anchor represents a page boundary, not a per-row scroll position
- an anchor MUST move only after a page has been successfully served
- `ClipboardHistoryPage.next_anchor` MUST mean "continue paging in the same direction from this
  page boundary"
- `ClipboardHistoryPage.has_more` MUST be derived from overfetch rather than guessed from
  `records.len() == limit`
- loading newer clipboard commits at the head of history MUST NOT silently invalidate already
  issued anchors for older pages within the same logical history view
- `EventId` MUST remain a stable lookup key for detail queries even if a row is no longer resident
  in the GUI's bounded page cache
...
This crate replaced `nooboard-app` as the application runtime boundary.

After the GUI cutover:

- `crates/nooboard-app` is deleted
- `crates/nooboard-sync` is deleted
- `crates/nooboard-gui` depends on `nooboard-core` directly
