...
- `nooboard-network` MUST NOT import deleted legacy crates such as `nooboard-app`.
- `nooboard-network` MUST expose a standalone Rust API.
- `nooboard-core` owns config persistence and runtime orchestration.
...
## 19. nooboard-core Integration Contract

`nooboard-core` is required to:
...
- `nooboard-core` can manage direct seeds without `SettingsPatch::SetManualPeers`
