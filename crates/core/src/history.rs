...
pub(crate) async fn prune(store: &Store, settings: &Settings) -> Result<()> {
    let oldest = cutoff(settings, now_ms());
    let limit = settings.max_history_entries;
    store.run(move |db| db.prune_history(limit, oldest)).await
}
pub(crate) async fn query(
    ...
    let rows = store
        .run(move |db| {
            db.history(nooboard_storage::HistoryQuery {
                contains: &contains,
                limit,
                offset,
            })
