...
-> Result<(), TestError> {
    ...
    service
        .patch_settings(SettingsPatch::Network(NetworkSettingsPatch::SetListenPort(
            24001,
        )))
        .await?;
    let state_after_port =
        wait_for_state_update(&mut state_subscription, Duration::from_secs(2), |state| {
            state.settings.network.listen_port == 24001
                && state
                    .local_connection
                    .device_endpoint
        ...
        .await?;
    assert_eq!(state_after_port.settings.network.listen_port, 24001);
    assert_eq!(
        state_after_port
            .local_connection
            .device_endpoint
...
async fn identity_and_listen_port_patch_keep_running_sync_alive() -> Result<(), TestError> {
    ...
    service
        ...
        .await?;
    let _ = wait_for_service_state(service, Duration::from_secs(10), |state| {
    ...
    .await?;
    assert_eq!(state_after_device_id.identity.device_id, "desk-live");
    ...
    service
        ...
        .await?;
    let state_after_port = wait_for_service_state(service, Duration::from_secs(10), |state| {
        state.settings.network.listen_port == 24002
            && state
                .local_connection
                .device_endpoint
                .map(|endpoint| endpoint.port())
    ...
    .await?;
    assert_eq!(state_after_port.settings.network.listen_port, 24002);
