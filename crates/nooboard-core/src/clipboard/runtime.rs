...
use nooboard_platform::{ClipboardEvent, DEFAULT_WATCH_INTERVAL};
...
pub(crate) struct LocalClipboardObserved {
...
pub(crate) struct LocalClipboardSubscription {
...
impl LocalClipboardSubscription {
    ...
    pub(crate) async fn recv(
        &mut self,
    ) -> Result<LocalClipboardObserved, broadcast::error::RecvError> {
...
impl SuppressionState {
    ...
    fn should_suppress(&mut self, text: &str) -> bool {
        ...
        let should_drop = entries.pop_front().is_some();
        if entries.is_empty() {
...
impl ClipboardRuntime {
    ...
    pub(crate) fn write_text_with_suppression(&self, text: &str) -> CoreResult<()> {
        self.suppression
            .lock()
    ...
    pub(crate) fn start_watch(&self) -> CoreResult<()> {
        self.ensure_watch_started()
    }

    pub(crate) fn subscribe_local_changes(&self) -> LocalClipboardSubscription {
        let receiver = self
            .watch
            .lock()
            .unwrap_or_else(|poisoned| poisoned.into_inner())
            .events_tx
            .subscribe();
        LocalClipboardSubscription::new(receiver)
    }

    pub(crate) async fn stop_watch(&self) -> CoreResult<()> {
    ...
    fn ensure_watch_started(&self) -> CoreResult<()> {
        let mut state = self
            .watch
            .lock()
            .unwrap_or_else(|poisoned| poisoned.into_inner());
