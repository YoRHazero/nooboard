...
use gpui::{
    AnimationExt as _, AnyElement, InteractiveElement, IntoElement, ParentElement,
    StatefulInteractiveElement, Styled, Window, div, prelude::FluentBuilder as _, px, svg,
};
...
use crate::{
    ...
    workspace::view_state::{
        HomeRadarPeerViewState, HomeRadarVisualState, HomeSystemCoreViewState,
    },
};

use super::{
    RADAR_CENTER, RADAR_MAX_RADIUS, RADAR_MIN_RADIUS, RADAR_SIZE, WorkspaceView,
    components::radar_panel_shell,
};
...
impl WorkspaceView {
    ...
    fn radar_core_icon(&self, state: HomeRadarVisualState) -> AnyElement {
        match state {
            HomeRadarVisualState::Stopped => svg()
                .w(px(22.0))
                .h(px(22.0))
                .path(RADAR_POWER_ICON_SVG)
                .text_color(theme::accent_cyan().opacity(0.92))
                .into_any_element(),
    ...
    fn radar_core(&self, state: HomeRadarVisualState) -> gpui::Div {
    ...
    fn radar_peer_dot(
        &self,
        index: usize,
        peer: &HomeRadarPeerViewState,
        radar_state: HomeRadarVisualState,
    ) -> impl IntoElement {
        let accent = if peer.transferring {
            theme::accent_blue()
        } else {
            theme::accent_green()
        };
    ...
    pub(super) fn radar_panel(&self, snapshot: &HomeSystemCoreViewState) -> impl IntoElement {
