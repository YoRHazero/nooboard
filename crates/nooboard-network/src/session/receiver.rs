...
#[derive(Debug, Clone)]
pub(crate) struct FileReceiverLimits {
...
#[derive(Debug, Clone)]
pub(crate) struct IncomingFileRequest {
...
#[derive(Debug)]
pub(crate) struct FileReceiverStateMachine {
    limits: FileReceiverLimits,
    active: HashMap<u32, IncomingTransfer>,
}

impl FileReceiverStateMachine {
    pub(crate) fn new(limits: FileReceiverLimits) -> Self {
        Self {
            limits,
            active: HashMap::new(),
        }
    ...
    pub(crate) async fn register_file_start(
        &mut self,
        transfer_id: u32,
        file_name: &str,
        file_size: u64,
    ...
    pub(crate) async fn handle_file_chunk(
        &mut self,
        transfer_id: u32,
        seq: u32,
        data: &[u8],
    ...
    pub(crate) async fn handle_file_end(
        &mut self,
        transfer_id: u32,
        checksum: &str,
    ) -> Result<DownloadedFile, FileReceiveError> {
    ...
    pub(crate) async fn handle_file_cancel(
        &mut self,
        transfer_id: u32,
    ) -> Result<(), FileReceiveError> {
        self.abort_transfer(transfer_id).await
    ...
    pub(crate) async fn abort_transfer_with_flag(
        &mut self,
        transfer_id: u32,
    ...
    pub(crate) async fn handle_file_cancel_with_flag(
        &mut self,
        transfer_id: u32,
    ) -> Result<bool, FileReceiveError> {
    ...
    pub(crate) async fn collect_idle_actions(
        &mut self,
        timeout: Duration,
