            ...
            match listener.accept().await {
                Ok((stream, addr)) => {
                    ...
                    tokio::spawn(async move {
                        ...
                        .await
                        {
                            warn!("incoming connection closed: {error}");
                        }
                    });
                }
                Err(error) => {
        ...
        while let Some(peer_addr) = peer_rx.recv().await {
            ...
            tokio::spawn(async move {
                loop {
                    ...
                    match connect_and_run(cfg.as_ref(), peer_addr, outbound_rx, incoming.clone())
                        .await
                    {
...
async fn handle_incoming_connection(
    ...
    debug!("accepted peer {} ({addr})", hello.device_id);
...
async fn connect_and_run(
    ...
    debug!("connected peer {}", ack.device_id);
